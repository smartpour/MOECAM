...
FILECONTENTS *readFile(int n, int m, double* objectives)
{
	...
	f->points = realloc(f->points, sizeof(POINT) * m);
	for(int ii=0;ii<m;ii++)
		{
			...
			p->objectives = realloc(p->objectives, sizeof(OBJECTIVE) * n);
			for(int jj=0;jj<n;jj++)
			{
				...
				p->objectives[objective] = objectives[ii*n+jj];
			}
		}
...
void FreeFC(FILECONTENTS *fc)
{
	for (int ii = 0;ii < fc->nFronts;ii++) {
		for (int jj = 0;jj < fc->fronts[ii].nPoints;jj++)
		{
			free(fc->fronts[ii].points[jj].objectives);
			
		}
		free(fc->fronts[ii].points);
	}
	free(fc->fronts);
...
//n dimension, m number of points
double mainHV(int nn, int m, double* objectives, double* nadir) 
// processes each front from the file 
{
	...
	for (int i = 0; i < fc->nFronts; i++) {
		if (fc->fronts[i].nPoints > maxm) { 
			maxm = fc->fronts[i].nPoints;
		}
		if (fc->fronts[i].n       > maxn) {
			maxn = fc->fronts[i].n;
		}
	}
//	maxn = n;
...
// allocate memory
	...
	for (int i = 0; i < fc->nFronts; i++) {
		for(int j = 0; j < fc->fronts[i].nPoints; j++) {
			for(int k = 0; k < fc->fronts[i].n; k++) {
				fc->fronts[i].points[j].objectives[k] = fabs(fc->fronts[i].points[j].objectives[k] - ref.objectives[k]);
			}
		}
	}
