...
#include "wfg.h"
//#include "../hypervol/hypervol/pch.h"
...
void WFG::makeDominatedBit(FRONT ps, int p, int n)
...
double WFG::inclhv(POINT p, int n)
...
double WFG::inclhv2(POINT p, POINT q, int n)
...
double WFG::inclhv3(POINT p, POINT q, POINT r, int n)
...
double WFG::inclhv4(POINT p, POINT q, POINT r, POINT s, int n)
...
double WFG::exclhv(FRONT ps, int p, int n)
// returns the exclusive hypervolume of ps[p] relative to ps[0 .. p-1] 
{
	makeDominatedBit(ps, p, n);
	int N = n;
	double volume = inclhv(ps.points[p],n) - hv(fs[fr - 1], &N);
	fr--;
	return volume;
}

double WFG::hv(FRONT ps, int *n)
// returns the hypervolume of ps[0 ..] 
{
	// process small fronts with the IEA 
	switch (ps.nPoints) {
		case 1: 
			return inclhv (ps.points[0],*n); 
		case 2: 
			return inclhv2(ps.points[0], ps.points[1],*n); 
		case 3: 
			return inclhv3(ps.points[0], ps.points[1], ps.points[2],*n); 
		case 4: 
			return inclhv4(ps.points[0], ps.points[1], ps.points[2], ps.points[3],*n); 
	}
	TEMPN = *n;
	...
	if (*n == 3 && safe > 0) {
		...
		for (int i = safe; i < ps.nPoints; i++) {
			...
			volume += ps.points[i].objectives[*n] * exclhv(ps, i, *n);
		}
		(*n)++;
		TEMPN = *n;
		return volume;
...
void WFG::PrepareWFG(int maxn, int maxm)
{
	 m_n = maxn;
	 maxdepth = maxn - 2;
	 nextp = new int*[maxdepth+2];// malloc(sizeof(int*) * maxdepth);
	 for (int i = 0; i < maxdepth+2; i++) {
		 nextp[i] = new int[maxm];
	 }
	 prevp = new int*[maxdepth+2];
	 for (int i = 0; i < maxdepth+2; i++) {
		 prevp[i] = new int[maxm]; //(sizeof(int) * maxm);
	 }
	 firstp = new int[maxdepth];//malloc(sizeof(int) * maxdepth);
	 lastp = new int[maxdepth]; //malloc(sizeof(int) * maxdepth);
	 psize = new int[maxdepth];//malloc(sizeof(int) * maxdepth);

	 fs = new FRONT[maxdepth]; //malloc(sizeof(FRONT) * maxdepth);
	 for (int i = 0; i < maxdepth; i++) {
		 fs[i].points = new POINT[maxm]; //malloc(sizeof(POINT) * maxm); 
 //		fs[i].n = maxn;
 //		fs[i].nPoints = maxm;
		 for (int j = 0; j < maxm; j++) {
			 fs[i].points[j].objectives = new OBJECTIVE[maxn];// malloc(sizeof(OBJECTIVE) * (maxn - i - 1));
		 }
	 }
...
void WFG::FreeWFG()
{
	...
	for (int i = 0; i < maxdepth; i++) {
		for (int j = 0; j < maxm; j++) {
			delete[](fs[i].points[j].objectives);
		}
		delete[](fs[i].points);
	}
	delete[](fs);
...
double WFG::mainentryWFG(int M, int N, FRONT* front)
{
	fr = 0;
	m_n = N;
	maxm = M;
	...
	double vol = hv(*front, &m_n);
	return vol;
}

double WFG::mainentryWFG(int M, int N, double* ref, double* pts)
// processes each front from the file 
{
	...
	FRONT front;    // to keep the scaled points 
	...
	front.n = N;
	front.nPoints = M;
	front.points = new POINT[maxm];//malloc(sizeof(POINT)*maxm);

	// modify the objective values relative to the reference point 

		for(int j = 0; j < maxm; j++) {
			front.points[j].objectives = new OBJECTIVE[N];//malloc(sizeof(OBJECTIVE)*maxn);
			for(int k = 0; k < N; k++) {
				front.points[j].objectives[k] = fabs(pts[j*N+k]  - ref[k]);
			}
		}
		...
		// clear memory
		for (int j = 0; j < maxm; j++) {
			delete[](front.points[j].objectives);
		}
		delete[](front.points);
