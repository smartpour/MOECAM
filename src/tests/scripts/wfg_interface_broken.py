...
This module provides a Python wrapper around the WFG     def _parse_wfg_output(self, output_text):
...
class WFGHypervolume:
    ...
    def _parse_wfg_output(self, output_text):
        ...
        for line in output_text.strip().split('\\n'):
            line = line.strip()
            # Look for lines that contain hypervolume values (typically numeric)
            try:
                # WFG typically outputs just the hypervolume value per line
